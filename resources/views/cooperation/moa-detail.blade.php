...
<div id="MoA" style='display: none !important;'>
    ...
    <div class="form-group {{$errors->has('area_of_coop') ? 'has-error' : null}}">
        <label for="area_of_coop" class="control-label">Bidang Kerjasama MoA / Perjanjian Kerja Sama</label>
        <textarea name="area_of_coop" class="form-control" id="bid_kerma_moa"
                  placeholder="Bidang Kerjasama MoA / Perjanjian Kerja Sama" required>{{$cooperation['area_of_coop']}}</textarea>
    ...
    <div class="form-group {{$errors->has('file_name_ori') ? 'has-error' : null}}">
        ...
        @if($disabled == null)
            <input name="file_name_ori" id="fileinput-moa-doc" type="file" class="file" accept="application/pdf.pdf">
